...
which is minimized by $\lambda^*_T=(13/90,17/90)$. Notice $f(\lambda^*_T)=d_\text{obs}$. With Gaussian assumptions on the prior and data, we have $\pi_\Lambda^\text{post} \sim \exp(-T(\lambda_T^*))$. Figure 1 shows plots of $\lambda_S^*$ and $\lambda_T^*$ along with the relevant visualizations of the terms in $S$ and $T$.} 
...
actually represent postprocessed quantities from the solution of complex physical models, often %involving quire function evaluations, which are generally expensive due to the many parameters. 
...
\noindent where $C_A=AC_\Lambda A^\top$. We may expand $f$ linearly around some $\lambda \in \Lambda$ and write $f(\lambda)-f(\lambda_\text{prior})=\nabla f (\lambda)^\top (\lambda-\lambda_\text{prior})$ so that in a neighborhood  about $\lambda$ we may write $f(\lambda)-f(\lambda_\text{prior})=A (\lambda-\lambda_\text{prior})$, where $A=\nabla f (\lambda)^\top$, which is an $N \times M$ matrix. We may rewrite \eqref{eq:11} as
...
In order to fully determine the form of \eqref{eq:12}, we must determine $(AC_\Lambda A^\top)^{-1}$. We refer to \cite{Tarantola} for a similar Newton-based approach. We note that since we assume $\nabla f$ is inaccessible, forming the data-consistent deregularization term in this regime may require gradient approximation methods for $f$ or more general surrogate modeling.
