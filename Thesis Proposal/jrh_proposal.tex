...
The use of the $\mathbb{E}(\cdot)$ expected value notation is a mathematically precise way of expressing the fact that the authors view function evaluations as a random draw, in the sense that true function values $f(x)$ are perturbed by a random vector in an additive fashion. Indeed, others such as Smith \cite{Smith} make similar assumptions, viewing evaluations of $f$ as a black-box process which obscures the true value. Chen and Wild chose to drop the $\mathbb{E}(\cdot)$ notation in (3) in favor $\Tilde{f}(x;\xi)$, which is meant to abbreviate the objective function's expression in (3), using a $\Tilde{\cdot}$ tilde notation to denote noisy evaluations, while maintaining a clearly-expressed dependence on random vectors $\xi \in \Xi$. 


In this paper, we shall return to the notation introduced in (1), where the objective function is written $f(x)+\epsilon$. Notice that we choose to drop the noise's dependence on $x$ in this notation. We note that this is consistent with Chen and Wild's assumption (iii.) above: since the expected value of the additive noise is unbiased with respect to $x$, we may allow ourselves to think of $\epsilon$ as its own random process, which will not depend on $x$. We emphasize that the value of $\epsilon$ will change for different realizations of $f(x)$, but will not rely on $x$. Our test function $f$ will have additive noise which will perturb the $f(x)$ by a random value proportional to the operator norm of $f$.



Chen and Wild propose the ``STARS (STep-size Approximation in Randomized Search)" algorithm, which: uses small perturbations in the domain $\Lambda$ given by the addition of a random vector with components drawn from a normal distribution; computes the function value at the randomly perturbed point with additive noise from -- in our case -- a uniform distribution; and updates iterates using a Gaussian-smoothed finite-difference scheme for approximate gradient information in a gradient descent scheme. We will examine the STARS algorithm in much closer detail in the Methods and Algorithms section. For now, we point out that the algorithm requires the ability to evaluate the model $f+\epsilon$, and needs access to random draws from a normal distribution and uniform distribution. All in all, the algorithm can be implemented in about 10 lines of code in \texttt{python}; see Appendix A.
