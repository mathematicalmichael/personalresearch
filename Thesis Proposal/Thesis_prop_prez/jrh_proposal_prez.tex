...
For a nonlinear $f$, inversion via convex optimization may be performed if $f$ is linearized and Gaussian assumptions are met. How can deregularization be performed so that data-consistency may be achieved?
...
\item  The updated  PDF is crucial to prediction of plasma profiles % (e.g. ion/electron density or temperature) 
...
\begin{itemize}
	...
	\item We propose to analyze additive and multiplicative noise in the context of optimization methods for DCI. We consider the impact of noise on MAP points and updated priors, especially when optimization is performed in a derivative-free setting.
