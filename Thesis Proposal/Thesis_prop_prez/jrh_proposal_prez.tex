...
\footnotetext[1]{T. Butler et al, \textit{Combining Pushforward Measures and Bayes' Rule to Construct Consistent Solutions to SIPs}}
\footnotetext[2]{Tarantola, Albert, \textit{Inverse Problem Theory and Methods for Model Parameter Estimation}}
\footnotetext[3]{Stuart, Andrew., \textit{Inverse problems: A Bayesian perspective}}
...
\footnotetext[1]{Tarantola, Albert, \textit{Inverse Problem Theory and Methods for Model Parameter Estimation}}
...
\footnotetext[1]{Wildey et al, \textit{A Consistent Bayesian Approach for SIPs Based on Push-forward Measures}}
...
\begin{itemize}
	...
	\item The deregularization term in \eqref{eq:3} ensures the updated the initial distribution $\Lambda$ will only be regularized in directions uninformed by data.
...
For a nonlinear $f$, inversion via convex optimization may be performed if $f$ is linearized and Gaussian assumptions are met. What form should the deregularization term take to roughly preserve data consistency? Can we construct error bounds based on local linearization error?
...
In a noise-free setting, the optimization method for inversion is valid \footnotemark[1] for nonlinear $f$, so long as nonlinearities are not too strong around $\lambda_\text{prior}$ and Gaussian assumptions are met.

\begin{block}{Research Questions}

\begin{itemize}

		\item To what extent does additive or multiplicative noise invalidate the assumptions needed to perform DCI via optimization, particularly the linearization of $f$?
		
		\item We investigate how using DFO to solve for a data-consistent MAP point compares to standard DCI methods in this setting.
...
\footnotetext[1]{Jan-Peter Calliess, \textit{Lipschitz optimisation for Lipscitz interpolation}}
\footnotetext[2]{Kvasov and Sergeyev, \textit{Lipschitz gradients for global optimization in a one-point-based partitioning scheme}}
...
\footnotetext[1]{Wesson, \textit{Tokamaks}}
...
\footnotetext[1]{S. Ku et al, \textit{A new hybrid-Lagrangian numerical scheme for gyrokinetic simulation of tokamak edge plasma}}
\footnotetext[2]{Battaglia et al, \textit{Kinetic neoclassical transport in the H-mode pedestal}}
...
\begin{itemize}
\item DCI problem: Find the updated PDF of the uncertain parameters in the expansions of $p$ and $f^2$. 
\item  The updated  PDF is crucial to accurate prediction of plasma profiles.% (e.g. ion/electron density or temperature) 
%and transport as these properties are extremely sensitive to perturbations
%in the magnetic equilibrium

\end{itemize}

\end{itemize}

\footnotetext[1]{Takeda, T. and Tokuda, S., \textit{Computation of MHD equilibrium of tokamak plasma}}
\footnotetext[2]{Lao et al, \textit{Reconstruction of current profile parameters and plasma shape in tokamaks}}
...
Optimization methods for DCI make Gaussian and linear assumptions, which will model a noisy $f$ only to the extent that $f$ is approximately linear in the neighborhood of interest.
...
\begin{itemize}
	...
	\item We consider the impact of noise on MAP points and updated priors, especially when optimization is performed in a derivative-free setting.
...
\begin{itemize}
	\item Initial model functions + noise
        \item  DCI of a subgrid LES model for isotropic turbulent flow.  
