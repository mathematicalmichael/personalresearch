...
\begin{itemize}
		...
		\item To what extent does additive or multiplicative noise %invalidate the assumptions needed to perform 
		alter DCI via optimization? %particularly the linearization of $f$?
...
\begin{frame}
	...
	\item The dimensionless, axisymmetric  Grad-Shafronov equation: %is given by
...
\end{equation}
	...
	\item In equilibrium fitting to data \footnotemark[2]: $p$ and $f^2$ are expanded as basis functions  in $\psi_n$, with the constraint that $p$ and $f^2$ vanish at the boundary.
	\end{itemize}

\begin{block}{Research Question}
The updated  PDF $\pi^u_\Lambda$ is important to prediction of plasma profiles.

\noindent{\bf DCI problem}:Find the updated PDF of the uncertain parameters in the expansions of $p$ and $f^2$.  
 % (e.g. ion/electron density or temperature) 
...
% yields the exact soOptimization methods for DCI make Gaussian and linear assumptions, which will model %noisy 
%$f$ only to the extent that $f$ is approximately linear in the neighborhood of interest. 
