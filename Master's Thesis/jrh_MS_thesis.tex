...
\SetAlgoLined
  ...
  Form $(L_1^\text{update})_k$ and if $(L_1^\text{update})_k>L_1^\text{prior}$, set $L_1^\text{prior}=(L_1^\text{update})_k.$
  
  Store $(\lambda^{(k-1)}+\mu^*_ku^{(k)},g_k)$ and $(\lambda^{(k)},f_k)$ as samples in $\{s_i\}.$ 
  
  Set $k=k+1$\;}
	
	Use $\{s_i\}_{i=1}^{2M_\A +10}$ to form $\tilde{W}$ for AS.  
  
    Set $\tilde{V}_\A:=\tilde{V}_{1:P,1:j}^\top$ and form $h_\A$.
