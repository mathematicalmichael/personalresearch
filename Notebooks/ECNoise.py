...
def ECNoise(f, x_b, M, mult=False):
    ...
    for i in range(1, M):
        S[0, i] = ((np.math.factorial(i)**2.)/np.math.factorial(2*i)
                   )*(1./(M-i))*np.sum(T[:, i]**2, axis=0)

    # Don't need the first column (because first col. of T just holds function values!)
    S = S[:, 1:]
